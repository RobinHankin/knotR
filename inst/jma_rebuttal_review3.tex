...
\textcolor{blue}{The curves certainly are visually continuous across
  the nodes---but only in the optimized knots.  Figure 4, for example,
  illustrates an unoptimized knot and we see that the Bezier nodes can
  exhibit ugly features.  The motivation for keeping Bezier nodes far from crossing points 
I have added a
  figure of a trefoil knot that illustrates the reason why nodes'
  being at crossing points is undesirable.  In short, at the node of a
  Bezier curve, the radius of curvature undergoes a visually jarring
  discontinuity which looks bad, especially the understrand, which is
  particularly susecptible to such considerations because of the
  break.\\ \\ Also (refer to figure 5, which shows 7_6 with numbered
  strands), we can see that if a Bezier
