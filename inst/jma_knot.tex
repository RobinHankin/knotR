...
\item[{\tt midpoint\_badness()}] The idea is to suppress strands with
  ...
  point $c_ij$ of strand $i$ with strand $j$, we calculate the arc
  length $s=\int_{s=0}^{c_{ij}}\,ds$ from the node to each crossing
...
\item[{\tt node\_crossing\_badness()}] Somewhat complementary to {\em
  total crossing potential energy}, here we make the crossing points
  repel one another, as though they are all charged objects (with the
  same sign).  The idea is to spread out the nodes so no two nodes are
  too close together.  If crossing point $i$ is at position ${\mathbf
    p}_i$ we have $\sum_{i\neq j}\left|{\mathbf p}_i-{\mathbf
    p}_j\right|^{-1}$.
