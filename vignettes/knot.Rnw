...
knotplot2(jj,node=TRUE,lwd=3,circ=0) # top left
...
knotplot(jj,overunder_fig8,add=TRUE,gap=50)  # lower right
...
knotplot(jjA,ou=oA,add=TRUE,gap=50,lwd=4)
jjB <- sweep(as.inkscape(jjB),2,c(1500,-600),"+")
knotplot(jjB,ou=oB,add=TRUE,gap=50,lwd=4)
@ 
...
for(i in 1:3){
    for(j in 1:7){
        ...
        knotplot(k,b[[n]],add=TRUE,lwd=2,gap=30)
    }
}
@ 
